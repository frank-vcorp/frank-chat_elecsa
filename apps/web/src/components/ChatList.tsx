...
export default function ChatList({
  ...
  const [filterStatus, setFilterStatus] = useState<"all" | "human" | "ai">(
    "all",
  );
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const needsHumanConvs = convs.filter((c) => {
        ...
        if (branches.length > 0) {
          return (
            branches.includes(c.branch as any) ||
            c.branch === "general" ||
            !c.branch
          );
        }
        if (branch)
          return c.branch === branch || c.branch === "general" || !c.branch;
        return true;
      });
      ...
      if (
        prevNeedsHumanCountRef.current !== null &&
        currentCount > prevNeedsHumanCountRef.current
      ) {
  ...
  const handleDeleteConversation = async (
    ...
    if (
      !confirm(
        "¿Eliminar esta conversación? Se borrarán todos los mensajes, notas y alertas asociadas. Esta acción no se puede deshacer.",
      )
    ) {
      return;
    }
      ...
      <div className="p-4 border-b border-slate-800 bg-slate-900/50 backdrop-blur-sm sticky top-0 z-10 space-y-3">
        ...
        <div className="flex gap-2">
          <div className="relative group flex-1">
            <Search
              className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-500 group-focus-within:text-indigo-400 transition-colors"
              size={16}
            />
            <input
              type="text"
          ...
          <button
            ...
            className={`p-2.5 rounded-xl border transition-all ${
              showFilters || filterTags.length > 0 || filterStatus !== "all"
            ...
            {(isSupervisor || isAdmin) && (
              <div className="mb-4">
                ...
                <select
                  value={filterBranch}
                  onChange={(e) =>
                    setFilterBranch(e.target.value as BranchId | "all")
                  }
                ...
                >
                  ...
                  {Object.entries(BRANCH_NAMES).map(
                    ([id, name]) =>
                      id !== "general" && (
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {filteredConversations.map((conv) => (
          <div
            ...
            className={`group cursor-pointer p-4 hover:bg-slate-900/80 transition-all border-b border-slate-800/50 relative ${
              selectedConversationId === conv.id
                ? "bg-slate-900 border-l-4 border-l-indigo-500"
                  ...
                  <div className="flex flex-wrap gap-1 mt-2.5">
                    {conv.tags.map((tag) => {
                      ...
                      return (
                        <span
                          key={tag}
                          className={`text-[9px] font-medium px-1.5 py-0.5 rounded border ${colors[tag] || "bg-slate-800 text-slate-400 border-slate-700"}`}
                        >
                          {tag}
                        </span>
                      );
