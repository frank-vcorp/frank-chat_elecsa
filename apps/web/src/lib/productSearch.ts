...
export async function initializeSearchIndex(): Promise<MiniSearch<ProductDocument>> {
    ...
    try {
        ...
        console.log(`[MiniSearch] Indexación completada: ${docs.length} productos, ${synthesis.allBrands.size} marcas, ${synthesis.allFamilies.size} familias en memoria.`);
        return miniSearch;
...
export function invalidateSearchIndexCaches() {
    console.log('[MiniSearch] Invalidando índice y síntesis de RAM. Se reconstruirá en la próxima consulta.');
    global.searchIndexInstance = null;
    global.catalogSynthesis = null;
}
...
export async function listBrandsByCategory(categoryFilter?: string): Promise<string> {
    try {
        ...
        const sourcesInfo = matchedSources.length > 0 ? ` (familias: ${matchedSources.join(', ')})` : '';
