...
export default function AgentsPage() {
  ...
  useEffect(() => {
    if (agents.length > 0 && !selectedAgent) {
      const sofia =
        agents.find(
          (a) => a.type === "ai" && a.name.toLowerCase().includes("sofia"),
        ) || agents.find((a) => a.type === "ai");
      if (sofia) {
  ...
  const handleInitializeSofia = async () => {
    if (
      !confirm(
        "¿Deseas inicializar el agente Sofía con el prompt predeterminado?",
      )
    )
      return;
  ...
  const handleSavePrompt = async () => {
    ...
    try {
      ...
      setAgents(
        agents.map((a) => (a.id === selectedAgent.id ? { ...a, prompt } : a)),
      );
    } catch (error) {
                      ...
                      <div>
                        ...
                        {((agent.branches &&
                          ...
                          !(
                            ...
                            agent.branches[0] === "general"
                          )) ||
                          (agent.branch &&
                            agent.branch !== "general" &&
                            !agent.branches)) && (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-3 rounded">
                      ...
                      <p className="text-sm font-medium">
                        {selectedAgent.email || "No especificado"}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded">
                      <label className="block text-xs font-medium text-gray-500 mb-1">
                    ...
                    <div className="bg-gray-50 p-3 rounded">
                      ...
                      <p className="text-sm font-medium flex items-center gap-1">
                        ...
                        {selectedAgent.branch
                          ? BRANCH_NAMES[selectedAgent.branch] ||
                            selectedAgent.branch
                          : "General"}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded">
                      ...
                      <p className="text-sm font-medium">
                        {(selectedAgent as any).whatsapp || "No especificado"}
                      </p>
                    </div>
                  </div>
                  <div className="bg-gray-50 p-3 rounded">
                    ...
                    <p className="text-sm font-medium flex items-center gap-2">
                      <span
                        className={`w-2 h-2 rounded-full ${selectedAgent.active !== false ? "bg-green-500" : "bg-red-500"}`}
                      ></span>
                    ...
                    <div className="bg-gray-50 border rounded p-4">
                      <div className="flex items-center gap-2 text-amber-600 mb-3">
                        ...
                        <span className="text-sm font-medium">
                          Solo lectura - Solo administradores pueden editar
                        </span>
                      </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      El agente verá conversaciones de las sucursales
                      seleccionadas
                    </p>
                  </div>
                  <div>
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            setEditFormData({
                              ...editFormData,
                              branches: [
                                ...editFormData.branches,
                                id as BranchId,
                              ],
                            });
                          } else {
                            const newBranches = editFormData.branches.filter(
                              (b) => b !== id,
                            );
                            setEditFormData({
                              ...editFormData,
