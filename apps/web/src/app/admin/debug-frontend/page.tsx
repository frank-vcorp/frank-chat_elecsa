      ...
      <div className="bg-white p-6 rounded-lg shadow space-y-6">
        ...
        <div>
          <h2 className="font-semibold mb-2">
            Firestore Connection (Client SDK)
          </h2>
          <div
            className={`p-4 rounded ${status.firestore?.connected ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}
          >
            {status.loading ? (
              "Checking..."
            ) : (
              <>
                <p>
                  ...
                  {status.firestore?.connected ? "Yes" : "No"}
                </p>
                <p>
                  <strong>Docs Found:</strong> {status.firestore?.docsFound}
                </p>
                {status.firestore?.error && (
                  <p className="mt-2 text-red-600">
                    <strong>Error:</strong> {status.firestore.error}
                  </p>
                )}
              </>
            )}
