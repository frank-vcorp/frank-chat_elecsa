...
async function verifyAdminRole(
  ...
  try {
    ...
    if (!agentDoc.exists && email) {
      const agentQuery = await adminDb
        .collection("agents")
        .where("email", "==", email)
        .limit(1)
        .get();
    ...
    if (!agentDoc.exists) {
      console.error(
        `[sync-excel] Acceso denegado: Usuario ${email} (${userId}) no encontrado en la colección 'agents'`,
      );
  ...
  try {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      const mappedProduct = {
        ...
        price:
          typeof row["PRECIO 2"] === "number"
            ? row["PRECIO 2"]
            : parseFloat(row["PRECIO 2"]) || 0,
        currency: (row["MONEDA"] || "MXN").toString().trim(),
        supplier: (row["Proveedor"] || "").toString().trim(),
        stock:
          typeof row["Disponible"] === "number"
            ? row["Disponible"]
    ...
    return NextResponse.json(
      { error: error.message || "Error del lado del servidor" },
      { status: 500 },
    );
  }
}
