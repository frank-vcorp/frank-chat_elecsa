...
export async function GET() {
  ...
  console.log(
    `[/api/agents/fix] GET ejecutado por admin: ${authResult.userId}`,
  );
  try {
    ...
    snapshot.docs.forEach((doc) => {
      ...
      if (data.branch && !data.branches)
        issues.push("Tiene branch pero no branches (array)");
      if (data.type === "human" && !data.email)
        issues.push("Agente humano sin email");
...
export async function POST() {
  ...
  console.log(
    `[/api/agents/fix] POST ejecutado por admin: ${authResult.userId}`,
  );
  ...
  try {
    ...
    return NextResponse.json({
      message:
        fixes.length > 0
          ? "Agentes corregidos"
          : "No se encontraron agentes para corregir",
