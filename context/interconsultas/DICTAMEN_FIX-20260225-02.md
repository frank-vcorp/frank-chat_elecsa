...
1. **Concurrencia Serverless (Thundering Herd)**: En entornos Serverless como Vercel, el uso de la bandera global `isIndexing` combinada con un `while` loop (Polling) puede atrapar otras solicitudes generadas concurrentemente hasta causar un *Timeout* si la indexación inicial de Firestore tarda mucho en bajar los 12,500 documentos.
